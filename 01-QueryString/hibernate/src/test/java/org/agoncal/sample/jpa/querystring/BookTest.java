...
public class BookTest {
    ...
    public void initTransaction() {
        ...
        // doesn't work because of http://opensource.atlassian.com/projects/hibernate/browse/HHH-5713
...
//        findAllScifiBooksCriteriaQuery = em.createQuery(q);
    ...
    @Test
    @Ignore("doesn't work because of http://opensource.atlassian.com/projects/hibernate/browse/HHH-5713")
    ...
    public void shouldShowTheJPQLQuery() throws Exception {
        // NAMED QUERIES
        assertEquals("SELECT b FROM Book b ORDER BY b.id DESC", findAllBooksNamedQuery.unwrap(org.hibernate.Query.class).getQueryString());
        assertEquals("SELECT b FROM Book b JOIN b.tags t WHERE t = 'scifi' ORDER BY b.id DESC", findAllScifiBooksNamedQuery.unwrap(org.hibernate.Query.class).getQueryString());

        // CRITERIA QUERIES
        assertEquals("select generatedAlias0 from Book as generatedAlias0 order by generatedAlias0.id desc", findAllBooksCriteriaQuery.unwrap(org.hibernate.Query.class).getQueryString());
//        assertEquals("select generatedAlias0 from Book as generatedAlias0 order by generatedAlias0.id desc", findAllScifiBooksCriteriaQuery.unwrap(org.hibernate.Query.class).getQueryString());
    }
}